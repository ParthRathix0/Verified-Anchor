...
fn check_discharges_example_obligations() {
    ...
    assert!(stdout.contains('\u{2713}'), "missing proven-check marker: {stdout}");
    assert!(stdout.contains('\u{26a0}'), "missing unproven-check marker: {stdout}");
    assert!(stdout.contains("CheckAuthority"), "missing CheckAuthority: {stdout}");
    assert!(stdout.contains("authority.key() == crate::ID"), "missing unproven expr: {stdout}");
    assert!(
        stdout.contains("Unproven checks still run at runtime; they can only reject more, never less."),
...
fn deny_unproven_fails_when_a_struct_has_an_unproven_check() {
    ...
    let out = Command::new(bin)
        .args(["verified-anchor", "check", "-p", "verified-anchor-example",
               "--lean-dir", lean_dir().to_str().unwrap(), "--deny-unproven"])
        .current_dir(PathBuf::from(env!("CARGO_MANIFEST_DIR")).parent().unwrap()) // rust/
        .output()
        .expect("run cargo-verified-anchor");
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(!out.status.success(), "expected --deny-unproven to fail:\nSTDOUT:\n{stdout}");
