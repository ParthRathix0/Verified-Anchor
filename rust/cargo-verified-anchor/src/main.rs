...
fn parse_args() -> Result<Args, String> {
    ...
    let mut args = Args { crate_name: None, lean_dir: None, json: false, deny_unproven: false };
...
fn render_report(specs: &[generate::Spec], json: bool, deny_unproven: bool) -> String {
    ...
    if json {
        ...
        for (i, s) in specs.iter().enumerate() {
            ...
            let ups: Vec<String> = s.unproven.iter()
    ...
    out.push_str(&format!(
        ...
        specs.len(), specs.len(), total_unproven
    ));
    if total_unproven > 0 {
        out.push_str("Unproven checks still run at runtime; they can only reject more, never less.\n");
    }
    out
}
