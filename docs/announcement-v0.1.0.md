# Verified Anchor — formally verified account validation for Solana

Verified Anchor is a drop-in replacement for Anchor's `#[derive(Accounts)]` where every macro expansion comes with a Lean 4 proof that the generated Rust code satisfies a formally specified validation contract. Syntax is identical to stock Anchor (`Account<'info, T>`, `Signer<'info>`, `Program<'info, P>`, `Context<'a, 'b, 'c, 'info, T>`). The proven core covers signer / mut / owner / has\_one / seeds + bump / discriminator constraints — the M4 subset — and the init / close lifecycle. Four CVE-class account-validation bugs from real Solana incidents are reproduced as on-chain before/after tests; the verified versions reject the attacker on-chain.
