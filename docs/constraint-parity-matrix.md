...
| `constraint = <expr>` | `Constraint.expr (e : Expr)` | **Proven + escape hatch** | `Operand`/`Cmp`/`Expr` sublanguage, `evalExpr` (fail-closed, strict `and`/`or`); `genValidate_sound` at `M10Subset` when the expression and every data field it reads compile into it. `nat`/`int` compare numerically widened. Falls to the escape hatch on a function call, macro, multi-segment path, or a field behind/of a float or Borsh enum (unlocatable in `T::LAYOUT`). `VAError::ConstraintViolated { field, expr }`. |
